...
#[proc_macro_derive(StructMerge, attributes(merge_targets))]
pub fn derive_heap_size(ast: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let attribute = input
        .attrs
        .iter()
        .filter(|a| a.path.segments.len() == 1 && a.path.segments[0].ident == "merge_targets")
        .nth(0)
        .expect("merge_targets attribute required for deriving StructMerge!");
